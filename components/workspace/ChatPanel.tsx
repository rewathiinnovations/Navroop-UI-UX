...
function messageKey(message: ChatMessage, index: number) {
  const stamp = message.timestamp instanceof Date ? message.timestamp.toISOString() : String(message.timestamp);
  return `${stamp}-${index}`;
}
              ...
              <div className={cn('flex', message.type === 'user' ? 'justify-end' : 'justify-start')}>
                <div
                  className={cn(
                    ...
                    message.type === 'ai' && 'bg-[var(--studio-surface)] text-[var(--studio-fg)] border border-[var(--studio-line)]',
                    message.type === 'system' && 'bg-[var(--studio-bg)] text-[var(--studio-muted)] border border-[var(--studio-line)]',
                    message.type === 'error' && 'bg-rose-50 text-rose-900 border border-rose-200',
                    message.type === 'command' && 'bg-zinc-900 font-mono text-[12px] text-zinc-100',
                    message.type === 'file-update' && 'bg-[var(--studio-bg)] text-[var(--studio-muted)]',
                  )}
                >
                  {message.type === 'user' &&
                    (message.metadata?.source === 'visual-edit' || message.metadata?.source === 'comment') && (
                      <span
                        className={cn(
                ...
                >
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      rating === 'up' ? 'text-[var(--studio-accent)]' : 'text-[var(--studio-faint)] hover:text-[var(--studio-fg)]',
                    )}
                  >
                  ...
                  <button
                    ...
                    className={cn(
                      'inline-flex size-28 items-center justify-center rounded-8',
                      rating === 'down' ? 'text-[var(--studio-accent)]' : 'text-[var(--studio-faint)] hover:text-[var(--studio-fg)]',
                    )}
                  >
