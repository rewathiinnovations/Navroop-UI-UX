...
function Metric({ label, value, hint }: { label: string; value: string; hint?: string }) {
  return (
    <div className="rounded-10 border border-[var(--studio-line)] bg-[var(--studio-surface)] px-12 py-10">
      <p className="text-[11px] uppercase tracking-[0.06em] text-[var(--studio-faint)]">{label}</p>
      <p className="mt-4 text-[18px] font-medium text-[var(--studio-fg)]">{value}</p>
      {hint && <p className="mt-2 text-[11px] text-[var(--studio-muted)]">{hint}</p>}
    </div>
  );
}
  ...
  return (
    <div className="flex h-full min-h-0 flex-1 flex-col overflow-hidden bg-[var(--studio-bg)]">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-16 py-16">
        ...
        {audit && metrics && (
          <div className="mb-16 grid grid-cols-2 gap-8 sm:grid-cols-5">
            <Metric
              label="Bundle"
            ...
            <Metric label="A11y" value={String(metrics.a11yViolations)} />
            <Metric label="Unused deps" value={String(metrics.unusedDeps)} />
          </div>
        )}
        {audit && (
          <>
            ...
            {grouped.failures.length === 0 && (
              <p className="text-[13px] text-[var(--studio-muted)]">No open code issues.</p>
            )}
