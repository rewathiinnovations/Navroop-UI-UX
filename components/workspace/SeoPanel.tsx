...
export default function SeoPanel({
  ...
  const { audit, scanning, error, hasFiles, filesHint, scan, fixOne, fixAll, toggleIgnore } =
    useSeoAudit(projectId);
  const [busyId, setBusyId] = useState<string | null>(null);
  // `hasFiles`/`filesHint` come straight from `useSeoAudit`'s own poll (the same
  // `getLatestSeoAudit` call it already makes) — mirrors PublishPanel's
      ...
      <div className="flex flex-wrap items-center justify-between gap-12 border-b border-[var(--studio-line)] px-16 py-12">
        ...
        <div className="flex items-center gap-8">
          ...
          {/*
            ...
            `runSeoAudit` answers the files question itself, before it takes the
