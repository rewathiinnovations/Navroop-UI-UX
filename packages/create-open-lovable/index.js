...
program
  ...
  .version('1.0.0')
  .option('-s, --sandbox <provider>', 'Sandbox provider (e2b, modal, or daytona)')
...
async function main() {
  ...
  if (!['e2b', 'modal', 'daytona'].includes(config.sandbox)) {
    console.error(chalk.red(`\n❌ Invalid sandbox provider: ${config.sandbox}`));
    console.log(chalk.yellow('Valid options: e2b, modal, daytona\n'));
    process.exit(1);
  }
