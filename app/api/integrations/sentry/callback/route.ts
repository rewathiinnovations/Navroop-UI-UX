...
import { consumeSentryOauthState, exchangeSentryCode, listSentryOrgs } from '@/lib/integrations/sentry-oauth';
...
export async function GET(request: NextRequest) {
  ...
  if (!csrf || !code) {
    return NextResponse.redirect(new URL('/admin/integrations?sentry=oauth-error', appUrl()));
  }
  ...
  const next = new URL('/admin/integrations', appUrl());
