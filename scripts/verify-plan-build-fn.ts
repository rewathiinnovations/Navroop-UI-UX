...
try {
  ...
  // No sandbox estimate any more: the flat initial cost is the AI call alone.
  assert(
    calculateEventCost('initial', false) === AI_GENERATION_ESTIMATE,
    'initial is the AI estimate',
  );
  ...
  assert(
    created.data.plan?.sourceMessage === 'Build a bakery site with a menu',
    'createPlan sourceMessage',
  );
  const defaultEvents = await prisma.generationEvent.findMany({
    where: { projectId: created.data.id },
  });
  assert(
    defaultEvents.length === 1 && defaultEvents[0].kind === 'plan',
    'no code-gen on default create',
  );
  ...
  const skipEvents = await prisma.generationEvent.findMany({
    where: { projectId: skipped.data.id },
  });
  ...
  assert(
    plans[1].trigger === 'initial' && plans[1].sourceMessage === created.data.plan?.sourceMessage,
    'refine keeps initial trigger',
  );
  assert(
    JSON.stringify(plans[1].content).toLowerCase().includes('newsletter'),
    'feedback reflected',
  );
  ...
  const refineAfterApprove = await runWithActor(owner, () =>
    refinePlan(created.data.id, 'Too late'),
  );
  ...
  assert(
    initialStart?.promptContext.includes('Build a bakery site with a menu'),
    'approve initial uses initialPrompt',
  );
  ...
  await prisma.project.update({
    ...
    data: {
      phase: 'COMPLETE',
      lastCode:
        '<file path="src/App.jsx">export default function App(){return <h1>Bakery</h1>}</file>',
    },
  });
  ...
  assert(
    !pendingBlocked.ok &&
      pendingBlocked.status === 409 &&
      pendingBlocked.error === 'A build is already in progress',
  ...
  assert(
    !planningBlocked.ok &&
      planningBlocked.status === 409 &&
      planningBlocked.error === 'A plan is already pending',
  ...
  const follow = await runWithActor(owner, () =>
    requestFollowUpPlan(complete.data.id, followSource),
  );
  ...
  assert(
    JSON.stringify(follow.data.content).includes('src/App.jsx') ||
      JSON.stringify(follow.data.content).includes(followSource),
    'follow-up context includes file tree or message',
  );
  ...
  assert(
    approvedFollow.ok,
    `approve follow-up failed: ${'error' in approvedFollow ? approvedFollow.error : ''}`,
  );
  ...
  assert(
    !followStart?.promptContext.includes('ORIGINAL INITIAL PROMPT bakery'),
    'approve followup does not use initialPrompt',
  );
