...
function abort(reason: string, detail: string[]): never {
  ...
  console.error('  Exit 2 means a broken scan, not a secret finding: staged content went unexamined.');
...
function pathsDifferingFromIndex(): Set<string> {
  const out = git(['diff', '--name-only', '-z'], 'git could not compare the worktree with the index');
  return new Set(
    out
      .split('\0')
      .map((row) => row.trim())
...
const mode = explicit.length > 0 ? 'explicit' : args.includes('--staged') ? 'staged' : 'tree';
...
const dirtyInWorktree = mode === 'staged' && files.length > 0 ? pathsDifferingFromIndex() : new Set<string>();
...
for (const file of files) {
  ...
  const result = dirtyInWorktree.has(file) ? readStagedBlob(file) : readWorktreeFile(file);
...
if (findings.length > 0) {
  ...
  console.error('  Exit 1 means a real finding. Remove the value (use .env.local or the Integration store),');
...
if (mode !== 'staged') {
  try {
    execFileSync('gitleaks', ['detect', '--no-git', '--source', process.cwd()], { stdio: 'inherit' });
  } catch (error) {
    ...
    if (failure.code === 'ENOENT') {
      console.error('gitleaks is not installed — second pass skipped; the in-repo rules still ran.');
    } else {
