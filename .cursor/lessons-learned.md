...
- **Superseded [2026-08-21]:** The critical named here is gone with its dependency — `@e2b/code-interpreter` (and the `tar@7.4.3` it pulled) left the tree with the sandbox subsystem, so `pnpm audit` no longer sees that path. `minimatch@^10.2.2` is still in `devDependencies` for `test-exclude` (next entry but one), and `package.json` carries no `pnpm.overrides` block today, so read the same-major advice as the rule for adding one rather than a description of what is there. **Corrected [2026-08-24]:** that last sentence sent the reader to the wrong file. pnpm 11 never reads overrides from `package.json`; they live in `pnpm-workspace.yaml`, where this repo already keeps eighteen under `overrides:`, held equal to the lockfile by `tests/unit/pnpm-overrides-in-sync.test.ts`. Adding the same-major selectors to `package.json` is a silent no-op — the file is not consulted, `pnpm audit` stays red, and the next move looks like dropping the audit step this entry forbids dropping. The same-major advice itself stands; only its destination changes. The rest stands unchanged: `deepmerge-ts` may go to `^8`, do not bump Prisma major, do not `pnpm add` while `:3000` is up, do not remove the audit step from verify.
