...
Entries are **never deleted** — a deleted lesson gets re-learned, usually the expensive way.
When the code an entry describes stops existing, append exactly one marker bullet as that
entry's **last** bullet and leave its original three bullets untouched:

- `- **Superseded [YYYY-MM-DD]:** …` — the rule still binds, only its example moved. Point at
  the live equivalent so the rule is followable today.
- `- **Obsolete [YYYY-MM-DD]:** …` — the mistake is unreachable: the subsystem it describes is
  gone. Say what deleted it, and what the lesson still generalises to, if anything.

Either marker must **name a replacement**. "No longer applies" on its own turns the entry into
a riddle. An unmarked entry is a live instruction, so never mark one whose subject is still in
the tree — grep for it first. `tests/unit/lessons-learned-lifecycle.test.ts` enforces the shape
and re-checks that the subjects existing markers call dead are still dead.

---

(newest entries on top)

### [2026-08-28] — `\/` inside a TypeScript template that _is_ the iframe script is eaten

- **What happened:** Chai Adda Preview reported `Uncaught SyntaxError: Unexpected identifier '$'` with no file:line. The bundled site parsed. The visual-edit inspector did not: `/^image\/(png|jpeg|webp|gif)$/` had become `/^image/` plus a leftover `$`.
- **Root cause:** `INSPECTOR_SCRIPT` is a TypeScript template literal (`lib/visual-edits/inspector.ts`). In that outer template `\/` is just `/`, so the regex closes at `image/` and V8 reads the next `$` as an identifier. `\\s` in the same file was already double-escaped; this slash was not. `new Function(INSPECTOR_SCRIPT)` reproduces it without a browser.
- **Rule going forward:** A script that will run in the preview frame and is authored as a `` `…` `` must emit a slash in a regex as `\\/`, or use `new RegExp('…')`. Do not trust a single `\/`. Guard with `new Function` on the interpolated string (`tests/unit/preview-display.test.ts`).
- **Obsolete [2026-08-28]:** Visual editing was deleted (`lib/visual-edits/`, inspector inject). The slash-in-template rule still binds for any iframe script authored as a TypeScript template — live equivalent is `PREVIEW_LINK_INTERCEPTOR` in `lib/preview/html.ts`.

### [2026-08-27] — "the suite wrote this file" is a claim about authorship that a tree diff cannot make

- **What happened:** `pnpm run verify` went red with 413/413 files and 4421/4421 tests passing. The only failure was the repo-write guard's teardown, naming two files under `public/uploads` — a preview build and a checkpoint snapshot. No test wrote them: they were the fifth pair in a series the dev server on `:3000` had been writing to the same real project id since 14:58, and the only tests naming those paths assert strings. The guard reported them as the suite's pollution because they appeared between its before and after snapshots, which is the only thing it can actually observe.
- **Root cause:** The guard's rule set was designed around concurrent _editors_ (a source file added or saved mid-run is classified with `git`) but not around the concurrent _application_, which this project's own `single-dev-server` rule guarantees is running from the same checkout. `public/uploads` is gitignored **and** a state prefix, so a dev-server write hits every branch that fails the run, and the two obvious escapes are both wrong: allowlisting the path deletes the check, and re-running until the server happens to be idle is not a fix.
- **Rule going forward:** Do not quiet a write guard by widening its allowlist — earn the silence instead. `tests/setup/storage-dir-guard.ts` repoints `STORAGE_LOCAL_DIR` at a throwaway directory from the `globalSetup` process (whose env the worker pool inherits — verified, unlike `DATA_DIR`, which is redirected worker-side and stays invisible to the guard) and again from `vitest.setup.ts` after `env.ts` has loaded `.env.local` with `override: true`; only then does `resolveFencedPrefixes` stop attributing that prefix, and it resumes the moment the storage root points back inside the repository. A subtraction from a guard needs a mechanism that makes the subtracted case impossible, plus a test that fails when the mechanism goes missing — here `tests/unit/storage-fence.test.ts`, which asserts from inside a worker, because dropping the import would otherwise leave `lib/storage` on its `public/uploads` fallback _and_ blind the guard to it. Related trap in the same file: `path.relative()` returns an absolute path across Windows drives, and this checkout is on `D:` while `os.tmpdir()` is on `C:`, so a containment check that only looks for a leading `..` reads every temp directory as "inside the repo".

### [2026-08-22] — a "stale on client-side nav" claim must first rule out a key-forced remount

- **What happened:** The workspace header (project name + "Last saved") kept showing the previous project after a sidebar switch, and I concluded the mount-only `initializePage` never re-ran. That was right, but I asserted it without first checking whether a `key` on the render site already forced a remount — the exact case where my conclusion would have been "the bug is elsewhere" and any title-refresh effect I added would have been redundant.
- **Root cause:** Reasoning from "the effect has `[]` deps" alone, instead of also reading the render site for a `key={id}` / `key={projectId}` and grepping for any _other_ effect already keyed on the path param. The mount-only effect and "no remount" are two separate facts, and only together do they prove staleness.
- **Rule going forward:** Before claiming a client-side navigation leaves state stale, verify all three: (1) the render site has no `key` that forces a remount, (2) the state-setting effect really is mount-only (`[]`), and (3) no sibling effect keyed on `[projectIdFromPath]` (or the path param) already refreshes that state. Then add the minimal effect, initialising its ref to the mount-time path id so the mount stays a no-op and the fetch is not doubled. The fix in `GenerationWorkspace.tsx` (`loadedTitleProjectRef`) is the reference pattern.

- **What happened:** Replacing the header's inline version pills and the "Open preview options" dropdown left `WorkspaceTopBar.tsx` with `WorkspaceViewSwitch` gone from JSX, the `) : null}` closer of the `{projectId ? (…) : null}` dropped, and `PanelLeftClose` / `MoreHorizontal` imports vanished — three separate tsc/eslint failures, each found one at a time.
- **Root cause:** The menu block sat between a `{projectId ? (` opener and its `) : null}` closer, and the import list was alphabetically adjacent to the icons I was adding. Range edits that stop at the trigger/content boundary do not carry the surrounding syntax with them, and a re-read showed the boundary but not that the whole expression's close had gone with the cut.
- **Rule going forward:** After removing or replacing a block inside a JSX conditional or a JSX element list, re-read the whole enclosing expression — including the line after the removed block for a stray `) : null}` / `}` / `</>` — and grep the file for every symbol you edited away (`VersionPills`, `previewOpen`, `setRoomForVersions`, `openPreviewWindow`) plus every icon still used in JSX (`PanelLeftClose`, `MoreHorizontal`). Do not declare a JSX edit done on `tsc` alone: run eslint on the touched file too, since an orphaned closer is a syntax error tsc reports first but an unused/missing import is a lint error.

- **What happened:** A new project after Approve showed "Starting AI generation..." / "Analyzing your request..." / "No files yet." Chat said "Building your project...". Those two strings are the client's pre-stream `setGenerationProgress`, not server statuses. The live `:3000` log had `plan/approve` then no `generation.start` for that project — the previous DeepSeek run still held the slot.
- **Root cause:** `sendChatMessage` always wrote the edit-mode thinking banner. `hasExistingSite` could also go true from leftover `appliedCode` because `/project/[id]` stays mounted across switches. The generate route awaited `providerSlot.started` **before** creating the SSE body, so no status frame could overwrite the local copy for the whole queue wait. Last `✓ Compiled` in the `:3000` terminal was also before the DeepSeek `fullStream` patch, so that route was not live either.
- **Rule going forward:** First builds use `startingGenerationFields` (wait copy unless this project already has completed files). Drop `appliedCode` that is not from this `projectId` (`appliedCodeForSend`). Return SSE and send `WAITING_FOR_MODEL_STATUS` before `providerSlot.started`. To prove whether `:3000` loaded a generate-route edit, grep the terminal for `✓ Compiled` after the file write — if the last compile is older, do not treat the disk patch as live; ask the dedicated server owner to restart.
...
- **Superseded [2026-08-21]:** The rule and its mechanism are live — `settleStreamedGeneration` (`lib/jobs/settle-generation.ts`), `resumablePhaseFromEvidence` (`lib/jobs/resumable-phase.ts`) and the `sandbox_unavailable` code (`lib/jobs/copy.ts`) all still ship. Only the reason persist can be skipped changed: migration `20260819010000_drop_sandbox_columns` deleted the VM subsystem, so there is no FAILED/DEAD sandbox to fail on. The code kept its name and now means "the workspace for this project could not be started", covering three different pre-flight refusals — read "sandbox was FAILED or DEAD" as "pre-flight refused".
...
- **Superseded [2026-08-21]:** The Modal and Daytona write paths named here — `filesystem.writeText`, `absoluteSandboxPath`, `base64DecodeWriteCommand`, modal@0.9.0 itself — all went with the sandbox subsystem (migration `20260819010000_drop_sandbox_columns`); generated files are written in-process now (`lib/preview/server-bundle.ts`, `lib/checkpoints/snapshot`), with no shell in between. The escaping lesson still binds wherever a file body does reach a shell: the live example is the nginx `printf` in the generated Dockerfile (`lib/deploy/repo-files.ts`), which escapes its own newlines by hand for exactly this reason.
...
- **Superseded [2026-08-21]:** `/admin/sandbox-providers`, `lib/sandbox/test-run` and `lib/sandbox/provider-check-copy` were deleted with the sandbox subsystem, but the load-bearing half is unchanged and still live: `lib/security/url-guard-messages` is the client-safe twin of `lib/security/url-guard` (`node:dns`), `lib/jobs/copy` and `lib/import/errors` still import the messages module rather than the guard, and `tests/unit/client-import-boundary.test.ts` now walks the `'use client'` graph so this failure is a red test instead of a cold-compile 500 that a retry hides.
...
- **Superseded [2026-08-21]:** `teardownProvider`, `TeardownResult` and `sandbox.teardownLeaks` are gone — nothing is metered or billed per VM since migration `20260819010000_drop_sandbox_columns`, so the billed-stopped sentence has no subject left. The rule outlived its example and is enforced elsewhere: an integration's status may only be written by a check that exercised its own credentials (F-206 / F-212, see `AGENTS.md`), and publish and apply close through `lib/publish/failure-copy.ts` / `applyOutcome` rather than a sentence composed at the call site. Never write a claim whose proof you are not holding.
...
- **Superseded [2026-08-21]:** Two of the four examples died with the sandbox subsystem — there is no E2B `listFiles` and no Daytona reconnect to get `false` from. The other two are live and are the ones to copy: `readSnapshot` throws `SnapshotReadError` so publish cannot mistake an unreadable checkpoint for an empty one (`lib/publish/files.ts`, error code `snapshot_unreadable`), and a preview URL must parse as http(s) with a real hostname (`lib/preview/url.ts`). "A caught error became an innocent empty value" is the whole lesson and it did not move.
...
- **Superseded [2026-08-21]:** The `https://${getHost()}` interpolation was E2B's and went with the sandbox subsystem. Both halves of the rule are live: apply still closes through `applyOutcome` / `applyPageCopy` (`lib/generation/`) and nothing may compose a success line from `filesCreated.length`, and the URL half now lives in `lib/preview/url.ts`, which mints the signed preview URL on a distinct preview origin. `https://undefined` is still the shape to refuse.
...
- **Obsolete [2026-08-21]:** All four files (`restart-dev.ts`, `install-packages.ts`, `kill-sandbox`, `sandbox-status`) and the `declare global { var sandboxData: any }` declarations they shared were deleted with the sandbox subsystem, and `tests/unit/no-orphaned-scaffolds.test.ts` (F-766) now fails if `activeSandbox` / `sandboxState` / `existingFiles` are re-declared as globals at all — so this exact TS2403 collision is unreachable. What generalises: the first checked `declare global` declaration wins, so a stricter re-declaration in a shared helper poisons every other file. Prefer one shared type module over per-file `declare global`, and never tighten a global you do not own.

### [2026-08-18] — pre-commit scanned node.exe and failed open

- **What happened:** `.husky/pre-commit` invoked the secret-scan script via pnpm exec + tsx with the `--staged` flag. The script built its extra-path list from all of `process.argv`, so `argv[0]` (the node binary) survived the `--staged` filter. `--staged` never took effect. Meanwhile `git diff --cached` errors returned `[]` and printed `Secret scan passed (0 files).` exit 0.
...
- **What happened:** Running vitest via pnpm exec (with `--coverage`) exited 1 in ~5s without starting Vitest: `[ERR_PNPM_ABORTED_REMOVE_MODULES_DIR_NO_TTY] Aborted removal of modules directory due to no TTY`, after `[ERROR] Command failed with exit code 1: … pnpm.mjs install`.
- **Root cause:** pnpm runs a dependency-status check before `exec` / `run`. `node_modules` here does not match the lockfile (the nested `minimatch@10` under `test-exclude/node_modules`, plus the ignored overrides field (overrides live in `pnpm-workspace.yaml`, not `pnpm.overrides` in `package.json`), so pnpm decided to reinstall and asked to purge `node_modules` first. It aborted **only because an agent shell has no TTY** — that is luck, not a safeguard. Under a TTY, or with `CI=true` / `confirmModulesPurge=false`, it would have deleted the nested v10 graph and broken coverage entirely (see the test-exclude entry below).
- **Rule going forward:** Do not run `pnpm exec <tool>` in this repo while `:3000` is up. Invoke the installed binary directly: `node ./node_modules/typescript/bin/tsc --noEmit`, `node ./node_modules/eslint/bin/eslint.js . --max-warnings 0`, `node ./node_modules/vitest/vitest.mjs run --coverage`, `node ./node_modules/tsx/dist/cli.mjs <script>`. Scripts you _write_ still use pnpm; a script that must spawn a CLI resolves it with `createRequire(...).resolve('<pkg>/…')` and spawns `process.execPath` (see `scripts/migrate-test-db.ts`). Never set `CI=true` or `confirmModulesPurge=false` to get past the abort — that arms the deletion.
...
- **Rule going forward:** Use `pnpm-workspace.yaml` overrides with same-major selectors (`minimatch@3`, `glob@10`, `nanoid@3`). `deepmerge-ts` `^8` is OK — Prisma only calls `deepmerge` on plain config objects; do not bump Prisma major. Do not `pnpm add` / install while `:3000` is up. Do not remove the audit step from verify.
- **Superseded [2026-08-21]:** The critical named here is gone with its dependency — `@e2b/code-interpreter` (and the `tar@7.4.3` it pulled) left the tree with the sandbox subsystem, so `pnpm audit` no longer sees that path. `minimatch@^10.2.2` is still in `devDependencies` for `test-exclude` (next entry but one), and `package.json` carries no `pnpm.overrides` block today, so read the same-major advice as the rule for adding one rather than a description of what is there. The rest stands unchanged: `deepmerge-ts` may go to `^8`, do not bump Prisma major, do not `pnpm add` while `:3000` is up, do not remove the audit step from verify.
...
- **What happened:** An accidental npm fallback (instead of pnpm) created untracked `package-lock.json` next to `pnpm-lock.yaml`. Cursor warned it was using npm as the preferred package manager because multiple lockfiles existed and `npm.packageManager` was `auto`.
...
- **Rule going forward:** Use `deleteMany` for idempotent clears so not-found is a clean success and every remaining error is real. Log it, and where the failed step _is_ the alerting, report `ok: false` so `handleCron` turns it into a 500 and a failed `CronRun` row. Guards: `tests/unit/alert-clear.test.ts`, `tests/unit/alert-clear-failures.test.ts`.

### [2026-08-18] — Two halves of one pipeline reading different config stores

- **What happened:** Creating a project 500ed with "No AI provider is configured" on a deployment whose keys were all saved through the admin UI. The BUILD route resolved providers through `loadEffectiveProviderEnv` (personal → org → admin setting → env), but the PLAN path passed raw `process.env` to the same chain builder — so builds worked and project creation never could. Separately, both plan and build defaulted `requestedModel` to `appConfig.ai.defaultModel`, and a requested model is pushed to the _front_ of the chain, so the operator's configured primary (`ai.primaryProvider` / `AI_PRIMARY_*`) silently never led.
- **Root cause:** The env overlay and the client builder (`clientForEntry`) lived inside one route file instead of `lib/ai/`, so the second caller re-implemented the lookup against the wrong store; and a "default" argument that participates in _ranking_ is not a default, it is an override.
- **Rule going forward:** Provider resolution goes through `loadEffectiveProviderEnv` + `lib/ai/client-for-entry.ts` — any new caller of `requireUsableProviderChain` passes that env, and `requestedModel` is explicit-only (undefined when the user chose nothing). Guards: `tests/unit/generate-provider-preflight.test.ts`, `tests/unit/team-self-guard.test.ts` pattern for action-level refusals.

### [2026-08-18] — A saved-but-blank credential fails as the provider's 401, not as a config error

- **What happened:** Six E2B provider rows had accumulated with empty API keys (the create endpoint accepted `String(body.apiKey || '')`), and every boot on them failed with E2B's own "Unauthorized … authorization header is missing" — which reads like an SDK bug, not like "the row has no key". The Daytona SDK, when bundled by Turbopack, also failed at first file upload with a misleading runtime error (`form-data` dynamic require) until it was listed in `serverExternalPackages`.
- **Root cause:** Validation missing exactly where the operator could still see the empty field; and native-SDK dynamic requires do not survive bundling.
- **Rule going forward:** Refuse blank per-driver credentials at `POST /api/admin/sandbox-providers`; put SDKs with dynamic `require` in `serverExternalPackages`; when a provider error says a _header_ or _credential shape_ is missing (vs invalid), suspect the stored value is empty before suspecting the SDK.
- **Superseded [2026-08-21]:** `POST /api/admin/sandbox-providers`, the six blank E2B rows and the Daytona SDK all went with the sandbox subsystem (migration `20260819010000_drop_sandbox_columns`), so there are no per-driver credentials left to refuse. The rule moved to the credential stores that remain: `/admin/integrations` (`lib/integrations/`, AES-256-GCM, echoes `last4` only) and the settings registry (`lib/settings/`, DB row → env → fallback), both of which must refuse a blank secret at the write. The diagnostic half is the durable part — when a provider error names a _missing header or credential shape_ rather than an invalid one, suspect a stored empty string before suspecting the SDK.
