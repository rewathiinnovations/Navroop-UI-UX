...
- **Rule going forward:** Refuse blank per-driver credentials at `POST /api/admin/sandbox-providers`; put SDKs with dynamic `require` in `serverExternalPackages`; when a provider error says a *header* or *credential shape* is missing (vs invalid), suspect the stored value is empty before suspecting the SDK.
