...
- **Root cause:** Reasoning from "the effect has `[]` deps" alone, instead of also reading the render site for a `key={id}` / `key={projectId}` and grepping for any *other* effect already keyed on the path param. The mount-only effect and "no remount" are two separate facts, and only together do they prove staleness.
