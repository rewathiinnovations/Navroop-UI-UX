...
> "Where the scripts live" in `SKILL.md`. On Windows use `python` in place of `python3`.

```bash
# Find strategy by goal
python "$SKILLS_DIR/design-system/scripts/search-slides.py" "investor pitch" -d strategy

# Get emotion arc
python "$SKILLS_DIR/design-system/scripts/search-slides.py" "series a funding" -d strategy --json
```
