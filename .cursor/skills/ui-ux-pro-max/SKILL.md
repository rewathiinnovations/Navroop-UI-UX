...
The scripts and CSVs ship **inside this skill's own directory** — there is no copy in your home
profile, and a `~/.cursor/...` path would either fail or run a different, unversioned copy. Set
`SKILL_DIR` once per session, from the repository root, to the tree your agent loaded this skill
from:

```bash
SKILL_DIR=.claude/skills/ui-ux-pro-max   # Claude Code loads skills from .claude/skills/
SKILL_DIR=.cursor/skills/ui-ux-pro-max   # Cursor loads them from .cursor/skills/
```

PowerShell: `$SKILL_DIR = ".claude\skills\ui-ux-pro-max"`.

Every command below is written to run **from the repository root** through `$SKILL_DIR`, so
`--persist` writes `design-system/` under the project rather than inside the skill. The scripts
resolve their own `data/` directory from their location, so the working directory only decides
where output lands.

> **Note:** On Windows use `python` in place of `python3` — the python.org installer does not
> create a `python3` launcher.
...
```bash
python3 "$SKILL_DIR/scripts/search.py" "atlantafx primer enterprise theme" --stack javafx
python3 "$SKILL_DIR/scripts/search.py" "enterprise tableview density permission" --stack javafx
```
