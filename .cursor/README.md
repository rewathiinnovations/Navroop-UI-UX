...
- **Admin configuration** — `/admin/config` renders from `lib/settings/registry.ts` (`SETTING_GROUPS`/`SETTINGS`); adding a registry entry surfaces the setting. `lib/settings/resolve.ts` (server-only) resolves **DB → env → fallback**; clearing deletes the row (back to env). Secrets encrypted, echo `last4` only; `setting:`-prefixed `AppSetting` rows, 30s cache. `GET/PUT /api/admin/settings`; `POST /api/admin/settings/test` → `lib/settings/test-group.ts`, results typed `live` vs `local` — never present a presence check as a working key. Generation overlay: `lib/ai/effective-env.ts`. Tests: `tests/unit/settings-resolve.test.ts`.
