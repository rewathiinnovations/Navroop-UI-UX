...
export async function fetchProjectList(query: ProjectListQuery = {}) {
  ...
  try {
    response = await fetch(buildProjectsApiUrl(query));
  } catch (error) {
    // Two pages run this on a 4 s timer. An offline browser rejected here with a
    // TypeError nothing caught, so the tick died as an unhandled rejection and the
    // page kept showing a stale list with no explanation (F-446).
    if (isNetworkFailure(error)) {
      reportNetworkFailure();
      return { ok: false as const, status: 0, error: OFFLINE_MESSAGE };
    }
    throw error;
  }
  reportNetworkSuccess();
