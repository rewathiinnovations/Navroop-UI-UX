...
export function createSentryApi(
  ...
  async function sentryGet(path: string) {
    ...
    try {
      response = await fetchFn(`${SENTRY_API_BASE}${path}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      throw new SentryApiError(`Sentry API unreachable: ${message}`, { cause: error });
    }
    if (!response.ok) {
  ...
  return {
    async getProjectStats(): Promise<SentryProjectStats> {
      ...
      return {
        ...
        quota: {
          used,
          limit: Number.isFinite(quotaLimit) && quotaLimit > 0 ? quotaLimit : Math.max(used, 1),
          resetsAt,
        },
        topIssues,
      };
