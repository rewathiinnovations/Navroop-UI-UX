...
13. `knip`, scoped to `--include files` — **fatal** since 2026-08-25. It was report-only because its unused-file list ran to 271 entries, almost all of them F-448’s unreachable marketing tree, and a list that long blocks nothing anyone can act on. That tree is deleted and `knip.json` declares the script and Dockerfile entrypoints knip cannot infer, so the count is 0 and a newly orphaned file is a red gate. Scoped to files: the unused-_exports_ report is ~100 exported types that are a module’s published surface, and is still available by running knip bare
