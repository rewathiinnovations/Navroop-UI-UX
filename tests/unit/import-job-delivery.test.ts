...
describe('import route passes jobId so Firecrawl steps persist', () => {
  it('hands importJob.id into runProjectUrlImport', () => {
    const source = readFileSync(ROUTE, 'utf8');
    const start = source.indexOf('runProjectUrlImport({');
    expect(start).toBeGreaterThanOrEqual(0);
    const call = source.slice(start, source.indexOf('}),', start) + 2);
    expect(call).toMatch(/runProjectUrlImport\(\{/);
    expect(call).toMatch(/jobId:\s*importJob\.id/);
    expect(call).not.toContain('log.error');
  });
});
