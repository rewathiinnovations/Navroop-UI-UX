...
assert(
  byKey.mobile?.width === 390 && byKey.mobile?.height === 844,
  'mobile is iPhone 14/15 logical',
);
assert(byKey.tablet?.width === 820 && byKey.tablet?.height === 1180, 'tablet is iPad Air logical');
// `byKey.desktop?.width == null` used to stand here, and optional chaining made
// `undefined == null` true, so deleting the desktop device left this green (F-610).
assert(byKey.desktop !== undefined, 'desktop is one of the preview devices');
assert(
  byKey.desktop?.width === null && byKey.desktop?.height === null,
  'desktop fills available area',
);
...
assert(
  rotateDeviceSize(390, 844).width === 844 && rotateDeviceSize(390, 844).height === 390,
  'rotate swaps mobile w/h',
);
assert(
  rotateDeviceSize(820, 1180).width === 1180 && rotateDeviceSize(820, 1180).height === 820,
  'rotate swaps tablet w/h',
);
...
assert(
  parseStoredPreviewDevice(null).key === 'desktop' &&
    parseStoredPreviewDevice(null).rotated === false,
  'missing storage defaults to desktop',
);
assert(parseStoredPreviewDevice('mobile').key === 'mobile', 'plain key string is accepted');
assert(
  parseStoredPreviewDevice(JSON.stringify({ key: 'tablet', rotated: true })).rotated === true,
  'JSON stores rotate',
);
...
assert(
  isMobilePreviewFinding({ detail: 'Button name is missing (390px, button).' }),
  '390px a11y finding is mobile',
);
assert(
  isMobilePreviewFinding({ title: 'Tap targets too small on mobile' }),
  'mobile in title is mobile',
);
assert(
  !isMobilePreviewFinding({ detail: 'Color contrast (desktop, .hero).' }),
  'desktop-only finding is not mobile',
);
assert(
  !isMobilePreviewFinding({ title: 'Unused dependency', detail: 'lodash is unused' }),
