...
assert(
  nextRules.includes('summary_large_image'),
  'SEO rules require twitter:card summary_large_image',
);
...
assert(
  nextRules.includes('app/sitemap.ts') && nextRules.includes('robots.ts'),
  'NEXTJS rules name sitemap.ts + robots.ts',
);
assert(
  nextRules.includes('generateMetadata') || nextRules.includes('Metadata'),
  'NEXTJS rules name Metadata API',
);

const htmlRules = getSeoRules('STATIC_HTML');
assert(
  htmlRules.includes('robots.txt') && htmlRules.includes('sitemap'),
  'STATIC_HTML rules name literal robots/sitemap',
);

for (const stack of ['REACT'] as const) {
  const rules = getSeoRules(stack);
  assert(
    rules.includes('unreliable') ||
      rules.includes('social bots') ||
...
assert(
  nextPrefix.indexOf('QUALITY (every file)') < nextPrefix.indexOf('SEO / AEO'),
  'SEO block comes after BASE_RULES',
);

const reactPrefix = buildStablePromptPrefix('REACT', 'minimal');
assert(
  reactPrefix.includes('unreliable') || reactPrefix.includes('social'),
  'REACT prefix includes SPA honesty',
);
...
assert(
  byId['page-basics:html-lang']?.status === 'medium' ||
    byId['page-basics:html-lang']?.status === 'high',
  'missing html lang is a finding',
);
assert(
  byId['page-basics:viewport']?.status === 'medium' || byId['page-basics:viewport'],
  'missing viewport is a finding',
);
assert(
  missing.some((row) => row.category === 'metadata' && row.status !== 'pass'),
  'missing metadata is not pass',
);
assert(
  missing.some((row) => row.category === 'open-graph' && row.status !== 'pass'),
  'missing OG is not pass',
);
assert(
  missing.some((row) => row.id === 'sitemap:missing' && row.status === 'medium'),
  'missing sitemap is medium',
);
assert(
  missing.some((row) => row.category === 'robots' && row.status !== 'pass'),
  'missing robots is a finding',
);

const noindex = runSeoChecks(
  scanInput({
    live: {
      url: 'https://preview.example/',
      status: 200,
...
assert(
  noindex.some((row) => row.category === 'indexing' && row.status === 'high'),
  'sitewide noindex is high',
);

const blocked = runSeoChecks(
  scanInput({
...
assert(
  blocked.some((row) => row.category === 'robots' && row.status === 'high'),
  'blocked robots.txt is high',
);

const homepageError = runSeoChecks(
  scanInput({
    live: { url: 'https://preview.example/', status: 500, html: '', headers: {} },
  }),
);
assert(
  homepageError.some((row) => row.id === 'indexing:homepage-error' && row.status === 'high'),
  'homepage error is high',
);

// --- good marketing home ---
const good = runSeoChecks(
  scanInput({
    stack: 'STATIC_HTML',
    files: [
...
assert(
  good.some((row) => row.id === 'page-basics:html-lang' && row.status === 'pass'),
  'good html lang passes',
);
assert(
  good.some((row) => row.id === 'page-basics:viewport' && row.status === 'pass'),
  'good viewport passes',
);
assert(
  good.some((row) => row.category === 'structured-data' && row.status === 'pass'),
  'home JSON-LD passes',
);
assert(
  good.some((row) => row.id === 'sitemap:present' && row.status === 'pass'),
  'sitemap present passes',
);

// --- skip JSON-LD on dashboards ---
const dash = runSeoChecks(
  scanInput({
    stack: 'NEXTJS',
    files: [
      {
        path: 'app/dashboard/page.tsx',
        content: 'export default function Dashboard(){return <h1>Dashboard</h1>}',
      },
...
assert(
  !dash.some(
    (row) =>
      row.category === 'structured-data' &&
      row.status !== 'pass' &&
      row.detail.toLowerCase().includes('dashboard'),
  ),
...
assert(
  dash.every(
    (row) =>
      row.category !== 'structured-data' || row.status === 'pass' || !row.id.includes('dashboard'),
  ),
  'no structured-data failure keyed to dashboard',
);

// --- NEXTJS sitemap.ts counts ---
const nextScan = runSeoChecks(
  scanInput({
    stack: 'NEXTJS',
    files: [
      {
        path: 'app/page.tsx',
        content:
          'export const metadata = { title: "Harbor Bakery — sourdough in Portland", description: "Harbor Bakery bakes sourdough, seasonal pastries, and coffee in Portland. Order online or visit our Division Street shop today." }',
      },
      {
        path: 'app/sitemap.ts',
        content: 'export default function sitemap(){return [{url:"https://example.com/"}]}',
      },
      {
        path: 'app/robots.ts',
        content: 'export default function robots(){return {rules:{userAgent:"*",allow:"/"}}}',
      },
    ],
    live: null,
  }),
...
assert(
  nextScan.some((row) => row.id === 'sitemap:present' && row.status === 'pass'),
  'app/sitemap.ts counts as sitemap',
);
assert(
  nextScan.some((row) => row.category === 'robots' && row.status === 'pass'),
  'app/robots.ts counts as robots',
);
...
assert(
  merged.find((row) => row.id === 'metadata:title:/')?.ignored === true,
  'ignored ids persist across rescan',
);
assert(
  merged.find((row) => row.id === 'sitemap:missing')?.ignored === false,
  'non-ignored stay active',
);
...
assert(
  one.toLowerCase().includes('build') || one.includes('SEO'),
...
assert(
  all.includes('Title missing') && all.includes('Sitemap missing'),
