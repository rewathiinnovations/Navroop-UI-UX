...
import { extractMemoriesAfterGeneration, parseExtractedMemories } from '../lib/memory/extract';
...
assert(
  memoryContentSchema.safeParse('x'.repeat(501)).success === false,
  'content rejects over 500',
);
...
assert(
  ...
  'category rejects unknown values',
);
assert(
...
assert(
  normalizeMemoryContent('  Always   USE Inter  ') === 'always use inter',
  'normalize trims and lowercases',
);
assert(
...
assert(
  parsed.every((row) => row.scope === 'PROJECT'),
  'extraction forces PROJECT scope only',
);
...
assert(
  parseExtractedMemories(JSON.stringify([{ category: 'nope', content: 'x' }])).length === 0,
...
assert(
  mixed.block.includes('always use Inter for body text'),
  'workspace ACTIVE content is included',
);
...
assert(
  !mixed.block.toLowerCase().includes('landing page structure'),
  'skill content is not in the memory block',
);
...
const long =
  'Keep this durable preference about typography and spacing for the generated site. '.repeat(4);
...
assert(capped.block.includes('workspace 11'), 'the newest workspace entry is still injected');
assert(capped.block.includes('project 39'), 'the newest project entry is still injected');
assert(
  !capped.block.includes('project 0'),
  'oldest project entries can be dropped to fit the budget',
);
assert(
  capped.truncatedScopes.length > 0,
  'the truncated scopes are named so the warning can point at a list',
);
...
assert(
  prefixA.indexOf('QUALITY (every file)') < prefixA.indexOf('Brain memory'),
  'memory comes after base-rules',
);
assert(!prefixA.includes('You are the user'), 'volatile user message is not in the stable prefix');

let completeCalls = 0;
const extracted = await extractMemoriesAfterGeneration(
  'p1',
  { sourceMessage: 'always use Inter' },
  {
    ...
    complete: async () => {
      ...
      return JSON.stringify([
        { category: 'design', content: 'always use Inter', scope: 'PROJECT' },
      ]);
    },
    insertPending: async () => {
...
const skipped = await extractMemoriesAfterGeneration(
  'p1',
  { sourceMessage: 'always use Inter' },
  {
  ...
  { sourceMessage: 'prefer dark UI' },
  {
    ...
    insertPending: async (rows) => {
      ...
      assert(
        ...
        'extracted rows are PENDING',
      );
      assert(
        rows.every((row) => row.source === 'extracted'),
        'extracted rows are source extracted',
      );
      assert(
        rows.every((row) => row.scope === 'PROJECT'),
        'extracted rows stay PROJECT',
      );
    },
  },
);
