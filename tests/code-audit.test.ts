...
import {
  shouldSkipAiReview,
  selectFilesForAiReview,
  parseAiReviewJson,
} from '../lib/audit/ai-review';
import {
  mergeIgnoredFindings,
  sortFindings,
  asCodeFindings,
  asMetrics,
  emptyMetrics,
...
assert(
  tsFindings.every((row) => row.status === 'high'),
  'each tsc error is high',
);
assert(
  tsFindings[0].filePath === 'src/App.tsx' && tsFindings[0].line === 12,
  'tsc finding has file/line',
);
...
assert(
  lintFindings[0].filePath?.endsWith('src/Button.tsx') && lintFindings[0].line === 8,
  'eslint finding has file/line',
);
assert(lintFindings[0].category === 'lint', 'eslint category is lint');

const nextCfg = eslintConfigForStack('NEXTJS');
assert(
  nextCfg.includes('typescript-eslint') &&
    nextCfg.includes('jsx-a11y') &&
    nextCfg.includes('react-hooks'),
  'NEXTJS eslint config includes ts + jsx-a11y + react-hooks',
);
const htmlCfg = eslintConfigForStack('STATIC_HTML');
assert(htmlCfg === null || htmlCfg.length === 0, 'STATIC_HTML skips eslint config');

// --- depcheck + npm audit ---
const depcheck = parseDepcheckJson(
  JSON.stringify({ dependencies: ['lodash', 'moment'], devDependencies: [] }),
);
assert(depcheck.length === 2, 'depcheck unused deps become findings');
assert(
  depcheck.every((row) => row.category === 'dependencies'),
  'depcheck category is dependencies',
);

const audit = parseNpmAuditJson(
  JSON.stringify({
    vulnerabilities: {
...
assert(
  audit.find((row) => row.title.includes('lodash'))?.status === 'high',
  'npm audit critical → high',
);
assert(
  audit.find((row) => row.title.includes('axios'))?.status === 'high',
  'npm audit high → high',
);
assert(
  audit.find((row) => row.title.includes('qs'))?.status === 'medium',
  'npm audit moderate → medium',
);
assert(!audit.some((row) => row.title.includes('debug')), 'npm audit low is omitted');

// --- knip / ts-prune ---
const knip = parseKnipJson(
  JSON.stringify({
    files: ['src/unused.ts'],
...
assert(
  knip.some((row) => row.filePath === 'src/unused.ts'),
  'knip unused file is a finding',
);
assert(
  knip.some(
    (row) =>
      row.title.toLowerCase().includes('deadfn') || row.detail.toLowerCase().includes('deadfn'),
  ),
  'knip unused export is a finding',
);

const pruned = parseTsPruneOutput('src/old.ts:12 - leftover\n');
assert(
  pruned[0]?.filePath === 'src/old.ts' && pruned[0]?.line === 12,
  'ts-prune finding has file/line',
);

// --- tool fail never aborts ---
const info = toolFailedFinding('typescript', new Error('tsc not found'));
assert(
  info.status === 'low' && info.fixable === false,
  'tool fail is informational (low, not fixable)',
);
assert(info.category === 'tool', 'tool fail category is tool');
assert(
  info.detail.toLowerCase().includes('tsc not found') ||
    info.detail.toLowerCase().includes('not found'),
  'tool fail includes the real error',
);
...
assert(
  getStack('NEXTJS').buildCommand === 'npm run build',
  'NEXTJS deploy buildCommand comes from stacks registry',
);
assert(
  getStack('NEXTJS').deployType === 'node' && getStack('NEXTJS').port === 3000,
  'NEXTJS is a node deploy',
);
assert(
  getStack('REACT').deployType === 'static' && getStack('REACT').outputDir === 'dist',
  'REACT is static dist',
);
assert(
  getStack('STATIC_HTML').buildCommand == null && getStack('STATIC_HTML').outputDir === '.',
  'STATIC_HTML has no build',
);
...
assert(
  failBuild[0]?.status === 'high' && failBuild[0]?.detail.includes('Module not found'),
  'build FAIL is top high with real error',
);
...
assert(
  heavy.some((row) => row.id === 'bundle:total-js' && row.status === 'medium'),
  'total JS >300KB gz is medium',
);
assert(
  heavy.some((row) => row.status === 'medium' && row.detail.includes('150')),
  'chunk >150KB gz is medium',
);
assert(
  heavy.some((row) => row.category === 'bundle' && row.title.toLowerCase().includes('image')),
  'unoptimized image is medium',
);
assert(
  heavy.some((row) => row.id === 'bundle:code-split' && row.status === 'low'),
  'missing code-split on multi-route is low',
);
assert(
  heavy.every(
    (row) =>
      row.detail.toLowerCase().includes('sandbox') || row.title.toLowerCase().includes('sandbox'),
  ),
  'bundle findings labeled sandbox estimates',
);
...
assert(
  !light.some((row) => row.status !== 'pass' && row.id === 'bundle:total-js'),
  'small bundle does not fail total JS',
);
...
assert(
  axeFindings[0].status === 'high' && axeFindings[0].selector === 'img.hero',
  'axe finding includes selector',
);
...
assert(
  !deduped.some((row) => row.id.includes('image-alt')),
  'missing-alt is not reported in both SEO and a11y',
);
assert(
  deduped.some((row) => row.id.includes('button-name')),
  'unrelated a11y findings are kept',
);

// --- AI review skip + file cap ---
assert(
  shouldSkipAiReview(
    Array.from({ length: 20 }, (_, i) => finding({ id: `s${i}`, title: 'x', status: 'high' })),
  ) === true,
  'skip AI when static findings >= 20',
);
assert(
  shouldSkipAiReview(
    Array.from({ length: 19 }, (_, i) => finding({ id: `s${i}`, title: 'x', status: 'high' })),
  ) === false,
  'run AI when static findings < 20',
);
...
const picked = selectFilesForAiReview(files, [
  finding({ id: 'typescript:src/huge.tsx:1', title: 'err', filePath: 'src/huge.tsx' }),
]);
assert(
  picked.every((file) => file.path !== 'src/huge.tsx'),
  'AI review skips files static analysis already fully covered',
);
...
const aiParsed = parseAiReviewJson(
  JSON.stringify({
    findings: [
      {
        title: 'State grows unbounded',
        detail: 'items[] never reset',
...
assert(
  aiParsed[0]?.category === 'ai-review' && aiParsed[0]?.filePath === 'src/big.tsx',
  'AI review JSON becomes findings',
);
...
assert(
  merged.find((row) => row.id === 'typescript:a')?.ignored === true,
  'ignored ids persist across rescan',
);
assert(
  merged.find((row) => row.id === 'lint:b')?.ignored === false,
  'new findings are not ignored',
);
...
assert(
  sorted.map((row) => row.id).join(',') === 'c,e,a,b,d',
  'sort failures high→medium→low, passing, ignored',
);

assert(asCodeFindings(null).length === 0, 'asCodeFindings handles null');
assert(
  asMetrics({ bundleKb: 12.5, tsErrors: 2, lintErrors: 1, a11yViolations: 3, unusedDeps: 4 })
    .tsErrors === 2,
  'asMetrics reads counts',
);
assert(emptyMetrics().bundleKb === null, 'empty metrics start with null bundle');

// --- fix instructions include file/line ---
const one = buildFixInstruction(
  finding({
...
assert(
  one.toLowerCase().includes('build') || one.toLowerCase().includes('fix'),
  'single fix is a build instruction',
);
...
assert(
  all.indexOf('High issue') < all.indexOf('Medium issue') &&
    all.indexOf('Medium issue') < all.indexOf('Low issue'),
  'fix-all lists severity order high→medium→low',
);
...
const grouped = groupRecurringIssues([
  {
    findings: [
      finding({ id: '1', category: 'typescript', title: 'TS2322' }),
      finding({ id: '2', category: 'lint', title: 'hooks' }),
    ],
  },
  {
    findings: [
      finding({ id: '3', category: 'typescript', title: 'TS2304' }),
      finding({ id: '4', category: 'a11y', title: 'alt' }),
    ],
  },
  { findings: [finding({ id: '5', category: 'typescript', title: 'TS2322' })] },
]);
assert(
  grouped[0]?.category === 'typescript' && grouped[0]?.count === 3,
  'top recurring category is typescript with count 3',
);
assert(
  grouped.some((row) => row.category === 'lint' && row.count === 1),
  'other categories are counted',
);
