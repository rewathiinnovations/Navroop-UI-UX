...
> Commands below run **from the repository root** with `SKILLS_DIR` set to the skill tree your agent
> loaded this skill from — `.claude/skills` under Claude Code, `.cursor/skills` under Cursor. See
> "Where the scripts live" in `SKILL.md`.

**Brand:** the `brand` skill is not vendored in this repository (see `CLAUDE.md`), so it has no
path here. Where it is installed, its scripts are `scripts/inject-brand-context.cjs` and
`scripts/validate-asset.cjs` inside the `brand` skill directory.

**Tokens:**

```bash
node "$SKILLS_DIR/design-system/scripts/generate-tokens.cjs" -c tokens.json
node "$SKILLS_DIR/design-system/scripts/validate-tokens.cjs" -d src/
```
