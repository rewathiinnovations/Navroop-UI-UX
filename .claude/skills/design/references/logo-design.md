...
This key is billed. `scripts/logo/generate.py` calls the paid Gemini image API: one image per
run, or one per variant with `--batch N` (capped at 9 styles). `search.py` needs no key. Full
breakdown, and why this key is shared with the product, in the "Cost" section of `SKILL.md`.
